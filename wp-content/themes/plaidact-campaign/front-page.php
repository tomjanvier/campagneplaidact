...
$hero_title = plaidact_get_theme_text("hero_title", get_bloginfo("name"));
$hero_subtitle = plaidact_get_theme_text(
    "hero_subtitle",
    get_bloginfo("description")
);
$petition_url = esc_url(
    plaidact_get_theme_text("hero_primary_cta_url", "#petition")
);
$petition_text = plaidact_get_theme_text(
    "hero_primary_cta_label",
    __("Signer la pétition", "plaidact-campaign")
);
$learn_url = esc_url(
    plaidact_get_theme_text("hero_secondary_cta_url", "#breves")
);
$learn_text = plaidact_get_theme_text(
    "hero_secondary_cta_label",
    __("En savoir plus", "plaidact-campaign")
);
$share_page = home_url("/");
...
$share_links = [
    ...
    [
        ...
        "url" =>
            ...
            rawurlencode($hero_title . " " . $share_page),
    ],
    [
        "label" => __("Telegram", "plaidact-campaign"),
        "icon" => "tg",
        "url" =>
            "https://t.me/share/url?url=" . $share_url . "&text=" . $share_text,
    ],
    [
...
<section class="section hero" id="accueil">
	<?php if ($hero_video): ?>
		<video class="hero__media" autoplay muted loop playsinline preload="metadata" poster="<?php echo $hero_image
      ? $hero_image
      : ""; ?>">
	...
	<div class="hero__share-list" aria-label="<?php esc_attr_e(
     "Partage sur les réseaux",
     "plaidact-campaign"
 ); ?>">
		<?php foreach ($share_links as $share): ?>
			<a class="hero__share-link" href="<?php echo esc_url(
       $share["url"]
   ); ?>" target="_blank" rel="noopener noreferrer" aria-label="<?php echo esc_attr(
    $share["label"]
); ?>">
				<span aria-hidden="true"><?php echo esc_html($share["icon"]); ?></span>
				<span class="hero__share-tooltip"><?php echo esc_html(
        $share["label"]
    ); ?></span>
			</a>
		<?php endforeach; ?>
	...
	<div class="wrap hero__content">
		...
		<div class="hero__actions">
			<a class="plaidact-button" href="<?php echo $petition_url; ?>"><?php echo esc_html(
    $petition_text
); ?></a>
			<a class="hero__link" href="<?php echo $learn_url; ?>"><?php echo esc_html(
    $learn_text
); ?> <span aria-hidden="true">→</span></a>
		</div>
		<?php get_template_part("template-parts/sections/partners", null, [
      "in_hero" => true,
  ]); ?>
	</div>
</section>
...
<section class="section hero-newsletter">
	<div class="wrap">
		<?php echo do_shortcode("[plaid_newsletter_form]");
...
$order_string = (string) get_theme_mod(
    "campaign_section_order",
    "petition,breves,articles,rapport,social_wall,send_mail"
);
$order = array_filter(
    array_map("trim", explode(",", strtolower($order_string)))
);
$already_done = [];
